...
class AttentionModule(torch.nn.Module):
    ...
    def __init__(self, input_size):
        ...
        # ADD YOUR CODE HERE
        self.project_keys  = torch.nn.Linear(input_size << 1, input_size)
        self.project_query = torch.nn.Linear(input_size     , input_size)
        self.unproject     = torch.nn.Linear(input_size, 1)
    ...
    def forward(self, encoder_outputs, decoder_hidden_state, attn_mask=None):
        ...
        # ADD YOUR CODE HERE
        if isinstance(decoder_hidden_state, (list, tuple)):
            query = \
                self.project_query(decoder_hidden_state[0].transpose(0, 1)) + \
                self.project_query(decoder_hidden_state[1].transpose(0, 1))
        else:
            query = self.project_query(decoder_hidden_state.transpose(0, 1))

        keys  = self.project_keys(encoder_outputs)

        scores = self.unproject(torch.tanh(query + keys))
...
class RNNEncoderDecoderLMWithAttention(torch.nn.Module):
    ...
    def __init__(self,src_vocab_size, tgt_vocab_size, embd_dims, hidden_size, num_layers=1, dropout=0.1):
        ...
        self.enc_rnn = torch.nn.LSTM(
            num_layers=num_layers, hidden_size=hidden_size, bidirectional=True,
            input_size=embd_dims, dropout=dropout, batch_first=True
        )
        self.dec_rnn = torch.nn.LSTM(
    ...
    def log_probability(self, seq_x, seq_y):
        ...
        with torch.no_grad():
            ...
            for i in range(len(seq_y) - 1):
                ...
                total_log_prob += logits[-1, seq_y[i]]
    ...
    def attentions(self, seq_x, terminate_token, max_length):
        ...
        with torch.no_grad():
            ...
            for _ in range(max_length):
                logits, decoder_hidden_state, attn = self(
                    seq_x, None,
                    decoder_hidden_state,
                    output_attention=True
                )
                outputs.append(logits[-1].cpu().argmax().item())
                attentions.append(attn)
                if outputs[-1] == terminate_token: break
    ...
    def forward(self, inputs, decoder_inputs=None, decoder_hidden_state=None, output_attention=False):
        ...
        # ADD YOUR CODE HERE
        enc_embd = self.enc_embd(inputs)

        if decoder_hidden_state is None:
            attn_mask = (inputs != 0)
            encoder_outputs, enc_hidden = self.enc_rnn(enc_embd)
            decoder_hidden_state = (
                torch.zeros(1, enc_hidden[0].shape[-2], enc_hidden[0].shape[-1], device=self.device),
                torch.zeros(1, enc_hidden[0].shape[-2], enc_hidden[0].shape[-1], device=self.device)
            )
        else:
            encoder_outputs, attn_mask, decoder_hidden_state = decoder_hidden_state

        attn_scores = self.attention(encoder_outputs, decoder_hidden_state, attn_mask)

        values = torch.bmm(attn_scores, encoder_outputs)

        dec_outputs, decoder_hidden_state = self.dec_rnn(values)
        dec_raw_logits = self.dec_unproject(dec_outputs)
        dec_logits     = torch.nn.functional.log_softmax(dec_raw_logits.squeeze(0), dim=-1)

        if output_attention:
            return dec_logits, (encoder_outputs, attn_mask, decoder_hidden_state), attn_scores
        else:
            return dec_logits, (encoder_outputs, attn_mask, decoder_hidden_state)
...
def load_params(src_tokenizer, tgt_tokenizer):
    ...
    return rnn_enc_dec_attn_params, rnn_enc_dec_attn_data_params, rnn_enc_dec_attn_training_params